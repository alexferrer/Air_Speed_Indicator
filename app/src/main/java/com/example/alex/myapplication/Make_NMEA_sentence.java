...
Helper class to create a valid NMEA sentence to send to XCSoar for reading

**/

private String cksum(string sentence) {
...
while (i < senlen) {
  ...
  cksum = cksum ^ int( char(sentence.substring(i,i+1))) ; // cksum^ord(sentence[i:i+1])
  i = i + 1
}

cksum = Integer.toHexString(cksum) ;  //hex(cksum)[2:]  (no need to [2:], java return only the hex value) 
return String.valueof(cksum);
}
